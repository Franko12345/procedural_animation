...
class Acido(Weapon):
    ...
    levels = [
        dict(count=1, cd=2.4, r=52, dmg=6, life=3.0, desc='solta pocas que ferem no chao'),
        dict(count=1, cd=2.4, r=52, dmg=9, life=3.0, desc='+dano'),
        dict(count=1, cd=1.9, r=60, dmg=9, life=3.2, desc='+cadencia, +area'),
        dict(count=2, cd=1.9, r=60, dmg=9, life=3.2, desc='+1 poca'),
        dict(count=2, cd=1.6, r=74, dmg=13, life=3.5, desc='+area, +dano, +cadencia'),
    ]
    ...
    def tick(self, player, game, dt, st, level):
        ...
        for _ in range(n):
            tgt = game.nearest_enemy(player.pos, 420)
            base = tgt.pos if tgt else player.pos
